...
export function classifyLedgerGroup(name) {
    const n = name.toLowerCase();
    if (matchesAny(n, ['bank', 'hdfc', 'sbi', 'icici', 'axis', 'yes bank', 'kotak', 'idbi', 'pnb', 'canara', 'hsbc']))
        return 'Bank Accounts';
    if (matchesAny(n, ['cash']))
        return 'Cash-in-Hand';
    if (matchesAny(n, ['debtor', 'receivable', 'customer', 'trade receivables']))
        return 'Sundry Debtors';
    if (matchesAny(n, ['creditor', 'payable', 'vendor', 'supplier', 'trade payables']))
        return 'Sundry Creditors';
    if (matchesAny(n, ['sales', 'revenue', 'income', 'service income']))
        return 'Sales Accounts';
    if (matchesAny(n, ['purchase', 'cost of goods', 'cogs', 'stock', 'inventory', 'materials']))
        return 'Purchase Accounts';
    if (matchesAny(n, ['salary', 'wage', 'employee', 'staff']))
        return 'Indirect Expenses';
    if (matchesAny(n, ['rent', 'electricity', 'telephone', 'office', 'admin', 'misc']))
        return 'Indirect Expenses';
    if (matchesAny(n, ['tax', 'vat', 'gst', 'tds', 'cess']))
        return 'Duties & Taxes';
    if (matchesAny(n, ['fixed asset', 'depreciation', 'equipment', 'furniture', 'vehicle', 'computer', 'laptop', 'plant', 'machinery']))
        return 'Fixed Assets';
    if (matchesAny(n, ['capital', 'drawing', 'owner', 'proprietor']))
        return 'Capital Account';
    if (matchesAny(n, ['loan', 'overdraft', 'cc account']))
        return 'Bank OD Accounts';
    if (matchesAny(n, ['discount received', 'commission received', 'interest income', 'rental income', 'other income', 'refund']))
        return 'Indirect Incomes';
    if (matchesAny(n, ['round off', 'rounding']))
        return 'Rounding Off';
    if (matchesAny(n, ['outstanding', 'accrual', 'provision']))
        return 'Current Liabilities';
    if (matchesAny(n, ['prepaid', 'advance', 'deposit', 'suspense', 'clearing']))
        return 'Current Assets';
    if (matchesAny(n, ['cost of', 'material', 'consumable', 'raw material']))
...
function buildLedgerEntry(ledgerName, amount, isDebit) {
    const sign = isDebit ? '-' : '+';
...
function buildLedgerMaster(name, parent) {
...
function buildVoucherMaster(name) {
    return `        <TALLYMESSAGE xmlns:UDF="TallyUDF">
...
export async function collectInvoiceLedgers(invoices, invoiceLines) {
...
export async function collectJournalLedgers(items) {
    const ledgers = new Map();
...
export function buildSalesVoucher(invoice, lines) {
...
export function buildPurchaseVoucher(invoice, lines) {
    const date = formatDate(invoice.invoice_date);
    const partnerName = invoice.partner_id[1];
    const ref = invoice.name || invoice.ref || '';
...
function determineBankVoucherType(items) {
    const BANK_KEYWORDS = ['bank', 'cash', 'hdfc', 'sbi', 'icici', 'axis'];
    const isBankLedger = (name) => {
        const n = name.toLowerCase();
...
export function buildJournalVoucher(entry, items) {
...
export function buildEnvelope(reportName, company, tallyMessages) {
...
export function buildMastersEnvelope(company, ledgers) {
...
export function buildVouchersEnvelope(company, voucherXmls) {
    return buildEnvelope('Vouchers', company, voucherXmls);
}
//# sourceMappingURL=odoo-to-tally.mjs.map